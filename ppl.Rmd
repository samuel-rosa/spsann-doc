...
# Points per lag
set.seed(2001)
sample_a <- optimPPL(points = 49, candi = candi, cutoff = cutoff, lags = 7,
                     x.max = x.max, x.min = sqrt(2), y.max = y.max, 
                     y.min = sqrt(2), boundary = boundary, iterations = 50000,
                     plotit = TRUE, verbose = FALSE)
# Point-pairs per lag
set.seed(2001)
sample_b <- optimPPL(points = 49, candi = candi, cutoff = cutoff, lags = 7,
                     x.max = x.max, x.min = sqrt(2), y.max = y.max, 
                     y.min = sqrt(2), boundary = boundary, iterations = 50000,
...
tmp <- sapply(1:length(sim_real),
              function (i) sim_real[[i]]$data[sample_a[, 1]])
sample_a <- cbind(sample_a, tmp)
rm(tmp)
tmp <- sapply(1:length(sim_real), 
              function (i) sim_real[[i]]$data[sample_b[, 1]])
sample_b <- cbind(sample_b, tmp)
rm(tmp)
tmp <- sapply(1:length(sim_real),
              function (i) sim_real[[i]]$data[sample_c[, 1]])
sample_c <- cbind(sample_c, tmp)  
rm(tmp)
tmp <- sapply(1:length(sim_real),
              function (i) sim_real[[i]]$data[sample_d[, 1]])
