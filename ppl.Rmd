...
sampling form simulated realities and then fitting linear mixed models with the
...
```{r}
# Clean workspace and load required packages, load simulated realities created
# in a previous R section, and source user-defined functions
rm(list = ls())
gc()
require(pedometrics)
require(spsann)
...
square root of two times the grid cell size (`sqrt(2 * 1) = 1.414214`). We use
seven exponentially spaced lag-distance classes. The lag-distance cut-off is 
set as equal to the radius of the bounding box of the spatial domain. The
solution is to be found with a maximum of 50000 iterations. We use three 
samples to have more confidence about the results.
...
Their definition is based on the study of [Lark (2002)]. The combination of the
different values results in nice scenarios. The range parameter of the variogram
model goes from very short (5 units) to very large (500 units) spatial 
dependence. We do not expect any sample configuration to enable estimating the
true range parameter of 500 units because estimates above the radius of the
spatial domain are highly uncertain. Our goal is to see the behaviour of the
different sample configurations in such an unfavorable case. The quantity of
spatially structured variance (partial sill) also varies from very little (0.1)
to very much (0.9).
...
the selection of the point locations, which guarantees that the three samples
will be different. This type of sample also produces a few clusters of points,
which is important to estimate the nugget variance. The resulting samples are 
checked visually and by computing the number of points and point-pairs per 
